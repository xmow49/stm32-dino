...
void ILI9341_INT_Fill(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color)
{
	...
	uint16_t pixel_per_send = 12;
	uint16_t datas[pixel_per_send];

	for (uint16_t i = 0; i < sizeof(datas) / sizeof(uint16_t); i++)
	{
		datas[i] = color;
	}
	...
	//	memset(frame, color, 240*320);
	//	SPI_WriteMultiNoRegister(ILI9341_SPI, frame, 10);
	//	SPI_WriteDMA(ILI9341_SPI, frame, 240*320);

	for (i = 0; i < pixels_count; i++)
	{
		SPI_WriteMultiNoRegister(ILI9341_SPI, datas, pixel_per_send);
	}
...
void ILI9341_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color)
{
	...
	{ /* vertical line */
		if (y1 > y0)
			ILI9341_INT_Fill(x0, y0, x0, y1, color);
		else
			ILI9341_INT_Fill(x0, y1, x0, y0, color);
		return;
	}
	...
	{ /* horizontal line */
		if (x1 > x0)
			ILI9341_INT_Fill(x0, y0, x1, y0, color);
		else
			ILI9341_INT_Fill(x1, y0, x0, y0, color);
		return;
	}
