...
@pre	pour utiliser les deux ports dans votre application, il faut avoir mis � 1 les deux defines SPI1_ON_PA5_PA6_PA7 et SPI1_ON_PB3_PB4_PB5
*/
 ...
 * @retval cette fonction retourne la donn�e lue sur le SPI choisi.
 */
 ...
 * @param SPIx est le SPI � lire.
 * @param *data est la variable qui va stocker les donn�es lues.
 * @param count est le nombre de donn�es � recevoir.
 */
 ...
 * @param SPIx est le SPI sur lequel envoyer la donn�e.
 ...
 * @param SPIx est le SPI sur lequel envoyer les donn�es.
 * @param *data est la donn�e � envoyer.
...
void SPI_WriteDMA(SPI_TypeDef* SPIx, uint8_t* data, uint16_t count){
	assert(SPIx == SPI1 || SPIx == SPI2);
	SPI_ID_e id = ((SPIx == SPI2)?SPI2_ID:SPI1_ID);
	spiok = 0;
	HAL_SPI_Transmit_DMA(&hSPI[id], data, count);
	while(!spiok){
		count++;
	}
}
 ...
 * @param SPIx le SPI dont on veut r�gler la taille des donn�es.
 * @param DataSize est la taille des donn�es :
